...
void internal_log_write(const char* level, const char* format, ...) {
    ...
    /* 检查INFO级别日志是否已禁用 / Check if INFO level log is disabled / Prüfen, ob INFO-Ebene-Protokoll deaktiviert ist */
    if (strcmp(level, "INFO") == 0) {
        pointer_transfer_context_t* ctx = get_global_context();
        if (ctx != NULL && ctx->disable_info_log != 0) {
            return; /* INFO日志已禁用，立即返回 / INFO log is disabled, return immediately / INFO-Protokoll ist deaktiviert, sofort zurückkehren */
        }
    }
...
int check_condition(const char* condition, void* param_value) {
    ...
    if (strcmp(condition, ">0") == 0) {
        ...
        /* 注意：假设param_value指向int类型，调用者应确保类型匹配 / Note: assumes param_value points to int type, caller should ensure type match / Hinweis: nimmt an, dass param_value auf int-Typ zeigt, Aufrufer sollte Typübereinstimmung sicherstellen */
