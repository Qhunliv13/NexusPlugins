...
int load_transfer_rules(const char* config_path) {
    ...
    /* 计算最大规则索引 / Calculate maximum rule index / Maximalen Regelindex berechnen */
    while (fgets(line_buffer, (int)line_buffer_size, fp) != NULL) {
        line_count++;
        char* trimmed_line = line_buffer;
        size_t len = strlen(trimmed_line);
        ...
        if (len >= 2 && trimmed_line[0] == '[' && trimmed_line[len - 1] == ']') {
            ...
            if (strncmp(current_section, "TransferRule_", 13) == 0) {
                ...
                if (endptr != NULL && *endptr == '\0' && parsed_index >= 0 && parsed_index <= INT_MAX) {
                    ...
                    if ((size_t)current_rule_index >= max_rule_index) {
                        max_rule_index = (size_t)current_rule_index + 1;
                    }
                } else {
    ...
    /* 检查calloc参数溢出 / Check calloc parameter overflow / calloc-Parameterüberlauf prüfen */
    if (max_rule_index > SIZE_MAX / sizeof(pointer_transfer_rule_t)) {
        internal_log_write("ERROR", "Rule index overflow detected: max_rule_index=%zu, sizeof(pointer_transfer_rule_t)=%zu, product would exceed SIZE_MAX", 
                     max_rule_index, sizeof(pointer_transfer_rule_t));
        free(line_buffer);
        fclose(fp);
        return -1;
    }
    
    pointer_transfer_rule_t* temp_rules = (pointer_transfer_rule_t*)calloc(max_rule_index, sizeof(pointer_transfer_rule_t));
    if (temp_rules == NULL) {
        free(line_buffer);
        fclose(fp);
        return -1;
    ...
    for (size_t i = 0; i < max_rule_index; i++) {
        temp_rules[i].source_param_index = -1;
        temp_rules[i].target_param_index = -1;
        temp_rules[i].transfer_mode = TRANSFER_MODE_UNICAST;
        temp_rules[i].enabled = 1;
    }
    
    rewind(fp);
    current_rule_index = -1;
    
    /* 解析规则配置项 / Parse rule configuration items / Regelkonfigurationselemente parsen */
    while (fgets(line_buffer, (int)line_buffer_size, fp) != NULL) {
        char* trimmed_line = line_buffer;
        size_t len = strlen(trimmed_line);
        if (len > 0 && trimmed_line[len - 1] == '\n') {
        ...
        if (current_rule_index >= 0 && (size_t)current_rule_index < max_rule_index) {
            char key[512];
            char value[2048];
            if (parse_key_value_simple(trimmed_line, key, sizeof(key), value, sizeof(value))) {
                trim_string(key);
                ...
                pointer_transfer_rule_t* rule = &temp_rules[current_rule_index];
    ...
    if (start_rule_index > SIZE_MAX - max_rule_index) {
        internal_log_write("ERROR", "Rule count overflow detected: start_rule_index=%zu, max_rule_index=%zu, sum would exceed SIZE_MAX", 
                     start_rule_index, max_rule_index);
        return -1;
    }
    size_t new_rule_count = start_rule_index + max_rule_index;
    ...
    internal_log_write("INFO", "Parsed %zu lines, found %zu rules, total rules: %zu", line_count, max_rule_index, ctx->rule_count);
    ...
    internal_log_write("INFO", "Loaded %zu transfer rules from %s", max_rule_index, config_path);
    return 0;
}
